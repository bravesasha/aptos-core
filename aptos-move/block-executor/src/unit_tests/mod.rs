...
use crate::{
    ...
    scheduler::{
        DependencyResult, ExecutionTaskType, Scheduler, SchedulerTask, TWaitForDependency, ValidationMode
    },
...
fn scheduler_tasks() {
    ...
    for i in 0..5 {
        ...
        assert_matches!(s.finish_execution(i, 0, ValidationMode::SelfOnly), Ok(SchedulerTask::Retry));
    }
    ...
    assert_matches!(s.finish_execution(4, 1, ValidationMode::SuffixAndSelf), Ok(SchedulerTask::Retry));
    assert_matches!(
        s.finish_execution(1, 1, ValidationMode::SelfOnly),
    ...
    assert_matches!(
        s.finish_execution(3, 1, ValidationMode::SelfOnly),
...
fn scheduler_first_wave() {
    ...
    assert_matches!(s.finish_execution(0, 0, ValidationMode::SelfOnly), Ok(SchedulerTask::Retry));
    ...
    assert_matches!(s.finish_execution(2, 0, ValidationMode::SelfOnly), Ok(SchedulerTask::Retry));
    // There should be no tasks, but finishing (1,0) should enable validating
    // (1, 0) then (2,0).
    assert_matches!(s.next_task(), SchedulerTask::Retry);

    assert_matches!(s.finish_execution(1, 0, ValidationMode::SelfOnly), Ok(SchedulerTask::Retry));
...
fn scheduler_incarnation() {
    ...
    assert_matches!(
        s.finish_execution(2, 1, ValidationMode::SuffixAndSelf),
    ...
    assert_matches!(
        s.finish_execution(0, 1, ValidationMode::SelfOnly),
    ...
    assert_matches!(
        s.finish_execution(2, 2, ValidationMode::SelfOnly),
    ...
    assert_matches!(s.finish_execution(4, 2, ValidationMode::SelfOnly), Ok(SchedulerTask::Retry));
...
fn scheduler_basic() {
    ...
    assert_matches!(s.finish_execution(0, 0, ValidationMode::SuffixAndSelf), Ok(SchedulerTask::Retry));
    assert_matches!(s.finish_execution(1, 0, ValidationMode::SuffixAndSelf), Ok(SchedulerTask::Retry));
    assert_matches!(s.next_task(), SchedulerTask::ValidationTask(0, 0, 0));
    assert_matches!(s.next_task(), SchedulerTask::ValidationTask(1, 0, 0));
    assert_matches!(s.finish_execution(2, 0, ValidationMode::SuffixAndSelf), Ok(SchedulerTask::Retry));
    assert_matches!(s.next_task(), SchedulerTask::ValidationTask(2, 0, 0));
...
fn finish_execution_wave() {
    ...
    assert_matches!(
        s.finish_execution(1, 1, ValidationMode::SuffixAndSelf),
...
fn rolling_commit_wave() {
    ...
    assert_matches!(s.finish_execution(2, 1, ValidationMode::SelfOnly), Ok(SchedulerTask::Retry,));
        ...
        loop {
            ...
            for _ in 0..num_tasks_to_perform {
                match tasks.pop_first().unwrap() {
                    (_, (true, txn_idx)) => {
                        let task_res = s.finish_execution(txn_idx, 0, ValidationMode::SuffixAndSelf);
                        num_exec_tasks += 1;
