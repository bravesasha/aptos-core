        ...
        self.execution.as_ref().and_then(|config| {
            let sizes = block_execution_times
                ...
                .flat_map(|summary| {
                    ...
                    if execution_time_ms > config.min_block_time_ms_to_activate as u128 && summary.payload_len > 0 {
            ...
            if sizes.len() >= config.min_blocks_to_activate {
                ...
                if max_block_txns > calibrated_block_size {
                    info!(
                        ...
                        "Execution backpressure recalibration: proposing reducing from {} to {}", max_block_txns, calibrated_block_size,
                    );
